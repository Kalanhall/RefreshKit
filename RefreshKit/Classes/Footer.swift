...
open class DefaultRefreshFooter:UIView, RefreshableFooter{
    ...
    #else
    public let indicator:UIActivityIndicatorView = UIActivityIndicatorView(activityIndicatorStyle: .gray)
    #endif
    ...
    override init(frame: CGRect) {
        ...
        textDic[.pullToRefresh] = RefreshKitFooterString.pullUpToRefresh
        textDic[.refreshing] = RefreshKitFooterString.refreshing
        textDic[.noMoreData] = RefreshKitFooterString.noMoreData
        textDic[.tapToRefresh] = RefreshKitFooterString.tapToRefresh
        textDic[.scrollAndTapToRefresh] = RefreshKitFooterString.scrollAndTapToRefresh
        udpateTextLabelWithMode(refreshMode)
    ...
    open override func layoutSubviews() {
        ...
        indicator.center = CGPoint(x: frame.width/2 - 70 - 20, y: frame.size.height/2)
    }
...
// MARK: - Private -
    fileprivate func addObservers(){
        ...
        attachedScrollView?.panGestureRecognizer.addObserver(self, forKeyPath:RefreshKitConst.KPathPanState, options:[.old,.new] , context: nil)
    }
    fileprivate func removeObservers(){
        ...
        attachedScrollView?.panGestureRecognizer.removeObserver(self, forKeyPath: RefreshKitConst.KPathPanState,context: nil)
    }
...
// MARK: - KVO -
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == RefreshKitConst.KPathOffSet && self.isUserInteractionEnabled{
            handleScrollOffSetChange(change)
        }
        guard !self.isHidden else{
            return;
        }
        if keyPath == RefreshKitConst.KPathPanState && self.isUserInteractionEnabled{
